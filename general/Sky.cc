...
Sky::Sky(double Lx, double Ly, double Lz, double lambda, double velocity)
    : Grid3D<AirCell>::Grid3D(ceil(1 + (Lx / lambda)), ceil(1 + (Ly / lambda)),
                              ceil(1 + (Lz / lambda)), lambda) {
...
Sky::Sky(PotentialField const& cp)
    : Grid3D::Grid3D(cp.getCellCount()[0], cp.getCellCount()[1], cp.getCellCount()[2],
                     cp.getCellSize()) {
    for (int i(0); i < cellCount[0]; ++i) {
        for (int j(0); j < cellCount[1]; ++j) {
            for (int k(0); k < cellCount[2]; ++k) {
...
std::ostream& Sky::printClouds(std::ostream& out) const {
    ...
    for (int i(0); i < cellCount[0]; ++i) {
        for (int j(0); j < cellCount[1]; ++j) {
            for (int k(0); k < cellCount[2]; ++k) {
                if (not(i == 0 or j == 0 or i == cellCount[0] - 1 or j == cellCount[1] - 1 or
                        k == 0 or k == cellCount[2] - 1)) {
                    out << i << " " << j << " " << k << " ";
